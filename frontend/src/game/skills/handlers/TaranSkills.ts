  ...
  'taran_secret_technique': (ctx) => {
    ...
    void engine; void hero; void dt; void validTargets; void healMult; void skillId;
    {
            ...
            if (target) {
              ...
              target.hp -= dmg;
              engine.addMeterDamage(hero.id, skillId, dmg);
              engine.addDamageNumber(target.position.x, target.position.y - 20, dmg, '#4c1d95');
              // 추가타
              for (let i = 0; i < 2; i++) {
                const bonus = Math.round(hero.atk * 2.0);
                target.hp -= bonus;
                engine.addDamageNumber(target.position.x + (i + 1) * 10, target.position.y, bonus, '#4c1d95');
              }
              return true;
            }
